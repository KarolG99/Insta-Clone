...
const Post = ({ id, username, userImage, img, caption }) => {
  ...
  return (
    <div className=" bg-white my-7 border rounded-md">
      ...
      {/* Post image */}
      <img src={img} className="object-cover w-full " alt="added image" />
        ...
        <div className="flex justify-between p-2 -mb-3 ">
          <div className="flex space-x-4">
            {hasLiked ? (
              <HeartIconField
                onClick={likePost}
                className="btn pt-1 text-red-500"
              />
            ) : (
          ...
          <BookmarkIcon className="btn pt-1" />
        </div>
      )}
        ...
        <div className="ml-10 h-20 overflow-y-scroll scrollbar-thumb-gray-200 scrollbar-thin">
          {comments.map((comment) => (
            <div key={comment.id} className="flex items-center space-x-2 mb-3">
              ...
              <Moment fromNow className="pr-5 text-sm text-gray-700">
      ...
      {session && (
        <form className="flex items-center p-4">
          ...
          <input
            ...
            className="flex-1 focus:ring-0 outline-none rounded-full mx-3 border-gray-200"
          />
          <button
